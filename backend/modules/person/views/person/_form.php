...
<div class="person-form">
    ...
    <?php if(!$model->isNewRecord){ 
		...
		?>
        	<span class="text-muted">[Note: Assigned role is deleted. Please select another role.]</span>
	<?php	}
	?>
    	<?= $form->field($model, 'role_id_fk')->dropDownList(ArrayHelper::map(Rolemanagement::find()->where(['isDeleted' => '0','is_administrator'=>'0','super_company_id_fk'=>$model->super_company_id_fk])->orderby('name ASC')->all(), 'role_id_pk', 'name'),['prompt'=>'--- Select One ---','class'=>'form-control','id'=>'gmp_role_id_fk','name'=>'gmp_role_id_fk'])->label(false); ?>

    <?php } else{ ?>
        	<?= $form->field($model, 'role_id_fk')->dropDownList(ArrayHelper::map(Rolemanagement::find()->where(['isDeleted' => '0','is_administrator'=>'0','super_company_id_fk'=>Yii::$app->user->identity->super_company_id_fk])->orderby('name ASC')->all(), 'role_id_pk', 'name'),['prompt'=>'--- Select One ---','class'=>'form-control','id'=>'gmp_role_id_fk','name'=>'gmp_role_id_fk'])->label(false); ?>
        <?php } ?>
    </div>
    </div>
    <?php } ?>
	...
	<div class="row">
        ...
        <div class="form-group col-sm-8">
            ...
            <?= Html::submitButton($model->isNewRecord ? 'ADD' : 'UPDATE', ['class' => $model->isNewRecord ? 'btn btn-primary' : 'btn btn-primary', 'id' => 'personsubbtn']) ?>
            <span class="text-muted">&nbsp; OR &nbsp;</span>
            <a href="<?= \yii\helpers\Url::to(['/person/person']) ?>" class="text-primary" style="text-decoration:underline;">Cancel</a>
            <?php } else { ?>
			<?= Html::submitButton($model->isNewRecord ? 'ADD' : 'UPDATE', ['class' => $model->isNewRecord ? 'btn btn-primary' : 'btn btn-primary','id' => 'personsubbtn', 'disabled'=>'disabled']) ?>
            <?php } ?>
            </div>
        </div>
    </div>
...
$this->registerJsFile(Yii::$app->homeUrl.'js/person.js', ['depends' => [\yii\web\JqueryAsset::className()]]);
?>
