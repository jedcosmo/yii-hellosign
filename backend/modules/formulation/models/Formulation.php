...
class Formulation extends \yii\db\ActiveRecord
{
	...
	public static function fPercentageTotal($provider, $fieldName,$mpr_defination_id='')
	{
		$total=0;
		$provider = parent::find()->where(['isDeleted'=>'0','mpr_defination_id_fk'=>$mpr_defination_id])->all();
		foreach($provider as $item){
			$total += floatval($item[$fieldName]);
		}
	...
	public static function kgWeightTotal($provider, $fieldName, $mpr_defination_id='')
	{
		$total=0;
		$provider = parent::find()->where(['isDeleted'=>'0','mpr_defination_id_fk'=>$mpr_defination_id])->all();
		foreach($provider as $item)
	...
	public static function showWbyW($data)
	{
