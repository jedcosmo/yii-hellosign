...
<div class="person-form">
    ...
    <?php if(!$model->isNewRecord){ ?>
    	<?= $form->field($model, 'role_id_fk')->dropDownList(ArrayHelper::map(Rolemanagement::findAll(['isDeleted' => '0','super_company_id_fk'=>$model->super_company_id_fk]), 'role_id_pk', 'name'),['prompt'=>'--- Select One ---','class'=>'form-control','id'=>'gmp_role_id_fk','name'=>'gmp_role_id_fk'])->label(false); ?>

    <?php } else{ ?>
        	<?= $form->field($model, 'role_id_fk')->dropDownList(ArrayHelper::map(Rolemanagement::findAll(['isDeleted' => '0','super_company_id_fk'=>Yii::$app->user->identity->super_company_id_fk]), 'role_id_pk', 'name'),['prompt'=>'--- Select One ---','class'=>'form-control','id'=>'gmp_role_id_fk','name'=>'gmp_role_id_fk'])->label(false); ?>
        <?php } ?>
	</div>
    </div>
    <?php } ?>
    ...
    <div class="col-sm-8">
    	<?= $form->field($model, 'country_id_fk')->dropDownList(ArrayHelper::map(Country::find()->where(['isDeleted' => '0'])->orderby('name ASC')->all(), 'country_id_pk', 'name'),['prompt'=>'--- Select One ---','class'=>'form-control','id'=>'gmp_country_id_fk','name'=>'gmp_country_id_fk','onchange'=> '$.get( "'.Yii::$app->urlManager->createUrl('person/person/lists?id=').'"+$(this).val(), function( data ) { 
    ...
    <div class="col-sm-8">
    		<?php if(!$model->isNewRecord){ ?>
        	 <?= $form->field($model, 'city_id_fk')->dropDownList(ArrayHelper::map(City::find()->where(['isDeleted' => '0','state_id_fk'=>$model->state_id_fk])->orderby('name ASC')->all(), 'city_id_pk', 'name'),['prompt'=>'--- Select One ---','class'=>'form-control','id'=>'gmp_city_id_fk','name'=>'gmp_city_id_fk'])->label(false); ?>
        <?php } else{ ?>
    		<?= $form->field($model, 'city_id_fk')->dropDownList(ArrayHelper::map(City::find()->where(['isDeleted' => '0','state_id_fk'=>$model->state_id_fk])->orderby('name ASC')->all(), 'city_id_pk', 'name'),['prompt'=>'--- Select One ---','class'=>'form-control','id'=>'gmp_city_id_fk','name'=>'gmp_city_id_fk'])->label(false); ?>
        <?php } ?>
    </div>
    ...
    <div class="row">
    <div class="col-sm-12"><div class="alert alert-warning">Please Note:<br/>1. Emailid/Username/Password changes will reflect after verification by email. <br/>2. Please check your emails after changing your profile details and click on the link from email to verify the changes. <br/>3. System will auto-logout on profile update.</div></div>
    </div>
	...
	<div class="row">
        ...
        <div class="form-group col-sm-8">
        	<div>
            <?= Html::submitButton($model->isNewRecord ? 'ADD' : 'UPDATE', ['class' => $model->isNewRecord ? 'btn btn-primary' : 'btn btn-primary', 'id' => 'myprofilesubbtn']) ?>
            <span class="text-muted">&nbsp; OR &nbsp;</span>
            <a href="<?= \yii\helpers\Url::to(['/']) ?>" class="text-primary" style="text-decoration:underline;">Cancel</a>
            </div>
        </div>
    </div>
...
$this->registerJsFile(Yii::$app->homeUrl.'js/myprofile.js', ['depends' => [\yii\web\JqueryAsset::className()]]);
?>
